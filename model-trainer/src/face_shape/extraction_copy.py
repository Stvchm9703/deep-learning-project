...
def batch_data(train: pandas.DataFrame, test: pandas.DataFrame):
...
if __name__ == "__main__":
    ...
    for idx, set_path in enumerate(set_list):
        print("try path " + set_path)
        tmp_path = extract_zip_data(set_path, DATASET_BASE)
        tmp_image_dataset = load_image_dataset(tmp_path, f"{DATASET_BASE}__batch_{idx}")
        train_set, test_set = train_test_split(tmp_image_dataset, test_size=0.3)
        batch_data(train_set, test_set)
        # dataset_list.append(tmp_image_dataset)

    # print(len(dataset_list))
    # %%
    # train, test = merge_and_shufsfle_data(dataset_list)
